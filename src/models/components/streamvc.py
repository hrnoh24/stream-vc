...
class EncoderBlock(Module):
    def __init__(
        ...
        residual_unit = partial(
            ResidualUnit, squeeze_excite=squeeze_excite, pad_mode=pad_mode
        )
...
class LocalTransformer(Module):
    def __init__(
        self, *, dim, depth, heads, window_size, dynamic_pos_bias=False, **kwargs
    ):
        super().__init__()
        ...
        *,
        ...
        for (chan_in, chan_out), layer_stride in zip(chan_in_out_pairs, strides):
            ...
            if use_gate_loop_layers:
                encoder_blocks.append(
                    Residual(ChannelTranspose(GateLoop(chan_out, use_heinsen=False)))
                )
...
class Decoder(Module):
    def __init__(
        ...
        *,
        ...
        self.decoder_init = CausalConv1d(
            embedding_dim, layer_channels[-1], 7, pad_mode=pad_mode
        )

        decoder_blocks = []
        for (chan_in, chan_out), layer_stride in zip(
            reversed(chan_in_out_pairs), reversed(strides)
        ):
